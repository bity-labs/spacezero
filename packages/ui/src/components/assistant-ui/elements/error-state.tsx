...
> {
  ...
  detail: string;
  retrying: boolean;
  /** Omit when no recoverable retry action exists; the Retry button is hidden. */
  onRetry?: () => void;
}
  ...
  if (retrying) {
    return (
      <div
        ...
        {...props}
      >
        <ArrowClockwiseIcon className="text-foreground/45 size-3.5 shrink-0 animate-spin motion-reduce:animate-none" />

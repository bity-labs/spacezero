...
export function ComposerMenu({
  ...
  return (
    <div
      ...
      className={cn(
        floating,
        "absolute bottom-full z-10 mb-2 flex flex-col gap-0.5 rounded-2xl p-1.5",
        width === "composer"
          ? "inset-x-0 w-full origin-bottom"
          : cn(
              "w-72",
              align === "start"
    ...
    >
      ...
      <span className="ms-1 flex w-5 items-center justify-end">
        {attachment.state === "uploading" ? (
          <SpinnerIcon className="text-foreground/35 size-3.5 animate-spin motion-reduce:animate-none" />
...
export function ComposerInput({
  ...
  return (
    <input
      ...
      className={cn(
        "placeholder:text-foreground/35 min-h-11 w-full bg-transparent px-3 text-[15px] caret-primary outline-none",
        className,
      )}
      {...props}
    />
        ...
        <div className="flex items-baseline justify-between">
          ...
          <p
            className={cn(
              mono,
              "tabular-nums",
              warn ? "text-destructive" : "text-foreground/35",
            )}
          >

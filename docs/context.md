...
- Skills use `<Space Zero Home>/skills` as the native global location and also support `~/.agents/skills`, project `.agents/skills`, and project `.pi/skills`. Project skills take precedence over global skills, and the chat composer exposes them through `/skill:name` discovery while Pi owns command expansion.

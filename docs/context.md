...
| Project Session | An agent session triggered from a Project that runs in its own Space Zero-managed Git worktree. | The builder's coding agent operating on an isolated project checkout. Uses Pi's built-in project tools against the managed worktree cwd plus Space Zero Workspace Tools. A durable GitHub Issue or Pull Request source link may explain and reopen the work that started it. |
...
- **Agent Definition scopes:** Definitions are discovered from four scopes — project (`<repo>/.agents/agents/`, walking up from the session cwd, trusted projects only), spacezero (`<spaceZeroHome>/agents/`, following the user-configured Space Zero Home), user (`~/.agents/agents/`), and bundled (defaults shipped inside the app, e.g. a read-only `scout` researcher and a `reviewer`). On ID collision the most specific scope wins: project > spacezero > user > bundled. Unlike skills, definitions need no enable/disable toggle because they are chosen explicitly rather than loaded ambiently.
- **Agent Definition vs Session:** An Agent Definition is static configuration (a markdown file); a Session is a live runtime instance backed by one Pi `AgentSession`. A definition may seed a session's model, thinking level, instructions, and tools, but the session remains a normal Space Zero Session afterwards.
...
- Space Zero has two session kinds: **Project Sessions** (triggered from a Project and run in a dedicated managed worktree cwd) and **Workspace Sessions** (global, spawnable at any time, e.g. from the Command Palette, with no required project context). Both are built from the same reusable Chat Components and backed by one Pi `AgentSession` each. Project Sessions use Pi's built-in project tools against the isolated worktree; Workspace Sessions primarily use Workspace Tools.
...
- Agent Definition discovery is lenient and diagnostic-driven: invalid files are skipped with reasons surfaced in Settings, and shadowed files are shown with their precedence status. Using a definition is loud: unknown models, empty tool allowlists, or reserved not-yet-supported fields (`output`, `blocking`) fail with explicit errors and never silently reinterpret the file's intent.

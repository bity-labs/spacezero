...
- Skills use `<Space Zero Home>/skills` as the native global location and also support `~/.agents/skills`, project `.agents/skills`, and project `.pi/skills`. Project skills take precedence over global skills, and the chat composer exposes them through `/skill:name` discovery while Pi owns command expansion. Settings provides global-skill enable/disable controls for the Space Zero Home and `~/.agents/skills` locations; disabled global skills are excluded from new or reloaded agent sessions, while project-local skills remain project-scoped.

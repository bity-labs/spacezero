  ...
  it('fetches a Pull Request ref with ephemeral askpass credentials and no token in argv', async () => {
    ...
    const runGit = vi.fn(
      async (request: {
        ...
        if (request.args.includes('rev-parse')) return { stdout: 'def456\n', exitCode: 0 }
        return { stdout: '', exitCode: 0 }
      }
    )
    const adapter = createManagedWorktreeAdapter({ runGit })
    ...
    expect(fetchRequest?.args.at(-1)).toMatch(/^refs\/pull\/79\/head:refs\/spacezero\/fetch\//)
  ...
  it('resolves concurrent Pull Request fetches from isolated temporary refs', async () => {
    ...
    const runGit = vi.fn(async (request: { args: string[]; allowFailure?: boolean }) => {
      ...
      if (request.args.includes('fetch')) {
        ...
        fetchHead = revision
        if (destinationRef) revisions.set(destinationRef, revision)
      ...
      if (request.args.includes('rev-parse')) {
        ...
        return { stdout: `${revisions.get(ref) ?? fetchHead}\n`, exitCode: 0 }
      }
      return { stdout: '', exitCode: 0 }
    })
    const adapter = createManagedWorktreeAdapter({ runGit })
    ...
    expect(
      requests
        .filter((request) => request.args.includes('fetch'))

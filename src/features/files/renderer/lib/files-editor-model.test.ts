...
describe('Files editor model helpers', () => {
  it('routes recognized filenames and extensions to Monaco languages with a plain-text fallback', () => {
    ...
    expect(getFilesEditorLanguage('native/main.cpp')).toBe('cpp')
    expect(getFilesEditorLanguage('native/header.hpp')).toBe('cpp')
    expect(getFilesEditorLanguage('rtl/logic.v')).toBe('verilog')
    expect(getFilesEditorLanguage('rtl/logic.vh')).toBe('verilog')

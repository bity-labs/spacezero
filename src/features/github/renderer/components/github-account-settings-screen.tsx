...
import type { GitHubConnection, GitHubDeviceAuthorization, GitHubInstallation } from '../../shared'
import { Alert, AlertDescription } from '@renderer/components/ui/alert'
import { Avatar, AvatarFallback, AvatarImage } from '@renderer/components/ui/avatar'
import { Button } from '@renderer/components/ui/button'
import { Card } from '@renderer/components/ui/card'

export type GitHubAccountSettingsScreenProps = {
...
export function GitHubAccountSettingsScreen({
  connection,
  isLoading,
  error,
  authorization,
  ...
  onOpenManageAccess,
  onDisconnect
}: GitHubAccountSettingsScreenProps): React.JSX.Element {
  if (isLoading) {
      ...
      {identity ? (
        <Card className="gap-4 p-5">
          ...
          {connection?.status === 'reconnect-required' ? (
            <Button className="w-fit gap-2" onClick={onStartAuthorization}>
            ...
            <div className="space-y-3">
              ...
              <div className="flex flex-wrap gap-2">
                <Button className="w-fit gap-2" onClick={onOpenInstallation}>
                  <ArrowSquareOut className="size-4" aria-hidden="true" />
                  Choose repository access
                </Button>
                <Button variant="outline" onClick={onCheckRepositoryAccess}>
          ...
          <div className="flex flex-wrap gap-2 border-t pt-4">
            {connection?.status !== 'reconnect-required' ? (
              <Button variant="outline" onClick={onOpenInstallation}>
                Add or change repository access
              </Button>
            ) : null}
            <Button variant="outline" onClick={onOpenManageAccess}>
        ...
        <Card className="gap-5 p-5">
          ...
          <div className="flex flex-wrap items-center gap-3">
            ...
            <Button variant="outline" className="gap-2" onClick={onCopyCode}>
              {copied ? <Check className="size-4" /> : <Copy className="size-4" />}
              {copied ? 'Copied' : 'Copy code'}
            </Button>
          </div>
          <div className="flex flex-wrap gap-2">
            <Button variant="outline" className="gap-2" onClick={onOpenAuthorization}>
              <ArrowSquareOut className="size-4" aria-hidden="true" />
              Open GitHub
            </Button>
            <Button variant="ghost" onClick={onCancelAuthorization}>
              Cancel
            </Button>
          </div>
...
function getConnectionLabel(connection: GitHubConnection | null): string {

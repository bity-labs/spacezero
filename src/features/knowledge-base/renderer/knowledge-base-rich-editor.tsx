...
import { splitMarkdownDocument } from './knowledge-base-markdown'
import './knowledge-base-rich-editor.css'
...
export function KnowledgeBaseRichEditor({
  ...
  const imageExtension = useMemo(
    () => createKnowledgeBaseImageExtension(documentRelativePath),
    [documentRelativePath]
  )
  ...
  const uploadImage = async (image: File): Promise<void> => {
    ...
    setImageError(null)
    if (image.size > MAX_KNOWLEDGE_BASE_IMAGE_BYTES) {
  ...
  return (
    <div className="knowledge-base-rich-editor">
      <input
        ref={imageInputRef}
        type="file"
      ...
      />
      <div
        className="knowledge-base-rich-editor__toolbar"
        role="toolbar"
      ...
      {imageError ? (
        <p className="knowledge-base-rich-editor__error" role="alert">
          {imageError}
        </p>
      ) : null}
      ...
      {linkEditorOpen ? (
        <div
          className="knowledge-base-rich-editor__link-panel"
          role="dialog"
          aria-label="Edit link"
        >
          <form
            className="knowledge-base-rich-editor__link-form"
            onSubmit={(event) => {
          ...
          {linkError ? (
            <p className="knowledge-base-rich-editor__link-error" role="alert">
              {linkError}
            </p>
          ) : null}
        </div>
      ...
      <EditorContent editor={editor} className="knowledge-base-rich-editor__content" />
    </div>
  )
}

function createKnowledgeBaseImageExtension(documentRelativePath: string) {
  ...
  const loadPreview = (markdownPath: string): Promise<string> => {
    ...
    const request = window.spacezero.knowledgeBase

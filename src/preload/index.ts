...
const api: SpaceZeroAPI = {
  ...
  files: {
    listTree: (request) => ipcRenderer.invoke(IPC_CHANNELS.files.listTree, request),
  ...
  knowledgeBase: {
    ...
    listChatHistory: () => ipcRenderer.invoke(IPC_CHANNELS.knowledgeBase.listChatHistory),
    resumeChatContext: (request) =>
      ipcRenderer.invoke(IPC_CHANNELS.knowledgeBase.resumeChatContext, request),
  ...
  github: {
    ...
    createOrReusePullRequest: (request) =>
      ipcRenderer.invoke(IPC_CHANNELS.github.createOrReusePullRequest, request),
  ...
  sessions: {
    ...
    getCurrentProjectChatContext: (request) =>
      ipcRenderer.invoke(IPC_CHANNELS.sessions.getCurrentProjectChatContext, request),
    listProjectChatHistory: (request) =>
      ipcRenderer.invoke(IPC_CHANNELS.sessions.listProjectChatHistory, request),
    resumeProjectChat: (request) =>
      ipcRenderer.invoke(IPC_CHANNELS.sessions.resumeProjectChat, request),
    clearProjectChat: (request) =>
      ipcRenderer.invoke(IPC_CHANNELS.sessions.clearProjectChat, request),
  ...
  update: {
    ...
    applyDownloadedUpdate: (request) =>
      ipcRenderer.invoke(IPC_CHANNELS.update.applyDownloadedUpdate, request),
    onStatusChange: (listener) => {
      const handler = (_event: IpcRendererEvent, payload: unknown): void => {
        listener(payload as Parameters<typeof listener>[0])
      }
      ipcRenderer.on(IPC_CHANNELS.update.statusChanged, handler)
      return () => ipcRenderer.off(IPC_CHANNELS.update.statusChanged, handler)
    }
  },
  browser: {
    ...
    clearData: () => ipcRenderer.invoke(IPC_CHANNELS.browser.clearData),
    onEvent: (listener) => {
      const handler = (_event: IpcRendererEvent, payload: unknown): void => {
        listener(payload as Parameters<typeof listener>[0])
      }

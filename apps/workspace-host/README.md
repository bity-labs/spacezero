...
The first implementation is a separate Local Host process launched under a private pinned Node runtime packaged with Desktop and reached over loopback. Closing the last Desktop window leaves it running, while explicitly quitting Space Zero stops local Sessions coherently and then stops the Local Host. The host remains independently runnable and testable without Electron so the same application and protocol can support future Space Zero-managed Remote Hosts.

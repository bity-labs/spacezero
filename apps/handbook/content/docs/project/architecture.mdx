---
title: Project Architecture
description: How Space Zero apps and packages fit together, then how the runtimes communicate.
---

# Project Architecture

Space Zero v0.1 is split into a trusted Desktop application and a separately executable Workspace Host.

The important rule is: **Desktop is the native shell and client; Workspace Host owns Project and Session execution.**

This page starts with the broad monorepo communication map, then drills down into runtime behavior step by step.

## 1. App and package overview

```mermaid
flowchart LR
  Desktop["apps/desktop\nElectron + React client"]
  ClientRuntime["packages/client-runtime\nBrowser-safe Host client"]
  HostContracts["packages/host-contracts\nShared Host Protocol contracts"]
  WorkspaceHost["apps/workspace-host\nHeadless Host runtime"]
  PiAdapter["packages/pi-adapter\nHost-side Pi boundary"]
  Handbook["apps/handbook\nPrivate docs app"]

  Desktop --> ClientRuntime
  ClientRuntime --> HostContracts
  WorkspaceHost --> HostContracts
  WorkspaceHost --> PiAdapter
  Handbook -. documents .-> Desktop
  Handbook -. documents .-> WorkspaceHost
```

At the package level, Desktop and Workspace Host do not import each other's application source. They meet through shared contracts and client/runtime packages.

- `apps/desktop` uses `packages/client-runtime` to talk to the Host.
- `packages/client-runtime` and `apps/workspace-host` both depend on `packages/host-contracts`.
- `apps/workspace-host` uses `packages/pi-adapter` for Pi-specific mechanics.
- `apps/handbook` documents the system, but does not participate in product runtime behavior.

## 2. Product communication flow

```mermaid
flowchart LR
  Builder["Builder"]
  Renderer["Desktop renderer\nReact UI"]
  ClientRuntime["Client Runtime"]
  Protocol["Host Protocol\nHTTP commands + SSE events"]
  Host["Workspace Host"]
  State["SQLite + Git worktrees + Pi"]

  Builder --> Renderer
  Renderer --> ClientRuntime
  ClientRuntime --> Protocol
  Protocol --> Host
  Host --> State
  Host --> Protocol
  Protocol --> ClientRuntime
  ClientRuntime --> Renderer
```

This is the normal product path. The user interacts with the React UI. The UI calls Client Runtime. Client Runtime sends authenticated HTTP/JSON commands to Workspace Host and listens for authenticated SSE events. Workspace Host performs the real Project, Session, Git, SQLite, and Pi work, then publishes events back to the client.

Electron main is not a proxy for ordinary Project or Session behavior.

## 3. Desktop runtime drill-down

```mermaid
flowchart TB
  subgraph Desktop["apps/desktop"]
    Main["Electron main\nNative lifecycle\nLocal Host supervisor"]
    Preload["Preload bridge\nNarrow window.spacezero API"]
    Renderer["React renderer\nUser interface"]
  end

  HostProcess["Workspace Host process"]

  Main -->|launches and supervises| HostProcess
  Main -->|delivers connection info\nand scoped client capabilities| Preload
  Preload -->|safe browser API| Renderer
  Renderer -->|native-only requests\nwhen needed| Preload
```

Desktop has three internal surfaces:

### Electron main

Owns app lifecycle, windows, native integration, Local Host launch/supervision, protected bootstrap, supervisor authority, and Desktop-local settings stored in app data.

### Preload

Exposes only a narrow, typed browser-safe bridge to the renderer. It can deliver Host connection information, short-lived scoped client capabilities, and validated Desktop-local settings APIs.

### Renderer

Owns UI only. It must not receive raw Node.js, Electron, filesystem, shell, SQLite, Git, Pi, or credential access.

## 4. Host Protocol drill-down

```mermaid
flowchart LR
  Renderer["React renderer"]
  ClientRuntime["packages/client-runtime"]
  Contracts["packages/host-contracts"]
  HostServer["Workspace Host\nHTTP/SSE handlers"]

  Renderer -->|plain client API| ClientRuntime
  ClientRuntime -. typed by .-> Contracts
  HostServer -. implements .-> Contracts
  ClientRuntime -->|Authorization header\nHTTP/JSON commands| HostServer
  HostServer -->|ordered SSE events| ClientRuntime
```

`packages/host-contracts` defines the stable wire shapes: schemas, endpoint declarations, event envelopes, command/query payloads, and public errors.

Client Runtime uses those contracts to call the Host. Workspace Host implements those contracts at the HTTP/SSE boundary. Contracts do not expose Electron types, React types, Pi SDK types, database rows, filesystem handles, or Host implementation internals.

## 5. Workspace Host drill-down

```mermaid
flowchart TB
  Server["Authenticated HTTP/SSE server"]
  Services["Host application services\nProjects + Project Sessions"]
  Database[("Host SQLite database")]
  Worktrees[("Managed Git worktrees")]
  PiAdapter["packages/pi-adapter"]
  PiSdk["Pi SDK"]

  Server --> Services
  Services --> Database
  Services --> Worktrees
  Services --> PiAdapter
  PiAdapter --> PiSdk
```

Workspace Host owns the Project catalog, Project Session behavior, event journal, relational projections, SQLite migrations, managed Git worktrees, and recovery policy.

Desktop can display and request this behavior, but does not duplicate or persist it. Desktop-local preferences such as language are kept separately in the Desktop settings JSON file, not in Host SQLite.

## 6. Pi boundary drill-down

```mermaid
flowchart LR
  Host["Workspace Host\nSpace Zero Session policy"]
  PiAdapter["packages/pi-adapter\nPi auth + conversation adapter"]
  PiSdk["Pi SDK"]
  Providers["LLM providers"]

  Host --> PiAdapter
  PiAdapter --> PiSdk
  PiSdk --> Providers
```

Workspace Host owns Space Zero Sessions. Pi Adapter owns Pi-specific mechanics: provider auth storage, conversation setup, SDK integration, event translation, cancellation, cleanup, and test seams.

Pi credentials and transcripts must not leak into Host Protocol contracts, SQLite Session events, logs, URLs, worktrees, or renderer persistent state.

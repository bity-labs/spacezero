...
The checked-in `electron-builder.yml` now contains the macOS beta Electron Builder target shape used by the repository release scripts and GitHub Actions workflow. It emits separate DMG/ZIP updater artifacts for `arm64` and `x64`, expects Developer ID signing plus App Store Connect API-key notarization in CI, embeds the configured Cloudflare R2 generic updater URL, and packages only the public GitHub App config from `resources/github-app.json`. Public R2 uploads remain guarded by an explicit repository variable until the packaged runtime is ready to ship.
